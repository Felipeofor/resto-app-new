 ...
 *
 * Two flows:
 *  1. OAuth / PKCE  -Üí ?code=...
 *  2. Email confirm -Üí ?token_hash=...&type=signup  (or recovery, invite, etc.)
 */
export async function GET(request: NextRequest) {
  ...
  if (token_hash && type) {
    ...
    if (verifyError) {
      ...
      return NextResponse.redirect(
        `${origin}/login?error=${encodeURIComponent('El link de confirmaci√≥n es inv√°lido o ya expir√≥. Intent√° registrarte de nuevo.')}`
      )
    }
      ...
      if (process.env.RESEND_API_KEY) {
        try {
          ...
          await resend.emails.send({
            ...
            to: user.email!,
            subject: '¬°Bienvenido a RestoQR! Tu panel est√° listo üö-',
  ...
  if (code) {
    ...
    if (exchangeError) {
      ...
      return NextResponse.redirect(
        `${origin}/login?error=${encodeURIComponent('Error al iniciar sesi√≥n. Intent√° de nuevo.')}`
      )
    }
